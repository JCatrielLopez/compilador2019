...
class Compiler {
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                ...
                case "-t":
                    ...
                    break;
                case "-h":
                    if (!(al_verbose || as_verbose || gc_verbose || ts_verbose))
                        print_help();
                    break;
                default:
    ...
    private static void print_help() {
        ...
        System.out.println(String.format("%6s %s %-30s", "-l", " ", "Print information from lexical analysis."));
        System.out.println(String.format("%6s %s %-30s", "-s", " ", "Print information from syntactic analysis."));
        System.out.println(String.format("%6s %s %-30s", "-g", " ", "Print information from assembler generation"));
        System.out.println(String.format("%6s %s %-30s", "-t", " ", "Print Symbol table"));
        System.out.println(String.format("%6s %s %-30s", "-h", " ", "Print help information. Fails if use with more arguments."));
    }
